...
- **Active work lives on the `admin-heroui` branch** (tracker:
  `ADMIN-HEROUI-MIGRATION.md`). `main` stays deployed and untouched as the
  rollback. Everything in the next two bullets ships on that branch.
- **Quote/contract scopes — the shared item model** (`src/lib/
  quoteTemplateConstants.ts` renders it for the admin preview, the customer
  quote page, and the email, so all three always agree):
  `bullet` · `note` · `fillable` (a fill-in "Label: value" line) · `choice`
  (admin-defined dropdown) · `subtotal` (a price line).
  **Every price line carries a billing cadence** — `billing: 'once' |
  'monthly' | 'yearly'`, absent = `once`. The three are summed into SEPARATE
  buckets and never added together: a $1,195 setup fee plus a $545/mo plan is
  never "$1,740". `sumSubtotalsByBilling()` + `renderQuoteTotalsHtml()` +
  `billingSummaryLine()` are the only places that decide how money reads.
  `paint_line` and the paint/stain/finish fields are RETIRED (painting-
  contractor leftovers) — old rows still render, no editor creates one.
  Known gap: the stored `quotes.total` column is still every subtotal minus
  discount, so internal stats and promoted projects overstate a monthly deal
  by one month. Fixing it needs a `recurring_total` column plus a pass over
  projects/contracts/payment schedules — Manny's call, not done.
- **Quote templates price both billing options.** `database/seeds/
  quote-templates.sql` is generated by `node scripts/gen-quote-template-seed.mjs`
  and gives every plan an "Investment — month to month" section AND an
  "Investment — prepay the year (two months free)" section (10× monthly,
  setup waived). Delete the one the customer isn't taking, or leave both.
  The seed deletes by name before inserting (re-running renumbers template
  ids and discards admin edits). NOT applied in production yet.
- **A promoted quote is a project, not a quote** (Manny, Aug 2026): the
  Quotes page excludes `status = 'project'` from its query, so it leaves
  every list and every stat the moment it's promoted. Never re-list it there.
- **Crew assignment stays, the crew portal doesn't** (Manny, Aug 2026): the
  crew-lead dropdown on a project is in use (he's the crew until he hires).
  The Crew Portal Access link block is out of the UI; the token, the
  `/project/crew/[token]` route, and the crew upload path are untouched.
- **Contract templates need a MannyKnows pass** (specified, not built):
  drop warranty months, keep late fees, rename the deposit to the first
  payment, and replace the N-payments scheduler with the two real billing
  modes (monthly plan / prepaid year), all per contract template.
- **Portfolios do not publish anywhere yet.** The admin's D1 `portfolios`
  tables and the public `/work/<slug>` case studies (content collection,
  `src/content/portfolio/*.md`) are unconnected, and `Make Portfolio` only
  flips a project's status. Full spec + the decision to make:
  `docs/PORTFOLIO-TO-WORK-HANDOFF.md`.
